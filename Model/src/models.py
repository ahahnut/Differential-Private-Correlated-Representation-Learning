...
class MISA(nn.Module):
    ...
    def alignment(self, sentences, visual, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        ...
        #Noise degree
        degree_s_t = ( torch.mean(self.utt_shared_t)/epsilon * (Correlation_dis_t + 1) ).cpu().detach().numpy()
        degree_s_v = ( torch.mean(self.utt_shared_v)/epsilon * (Correlation_dis_v + 1) ).cpu().detach().numpy()
        degree_s_a = ( torch.mean(self.utt_shared_a)/epsilon * (Correlation_dis_a + 1) ).cpu().detach().numpy()
